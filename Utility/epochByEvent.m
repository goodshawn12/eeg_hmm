function epoched_data = epochByEvent(data, event_latency, epoch_start_offset, epoch_end_offset)
    epoched_data = zeros(length(event_latency), size(data,2), epoch_end_offset - epoch_start_offset + 1); 
    for i = 1:length(event_latency)
        ...
        if sliced_epoch_end > size(data, 1)
            break;
        else
    ...
    epoched_data = squeeze(epoched_data);
end